   ...
   const handaleuserdelete = (email)=>{
    ...
    return (
      ...
      <td> {role !== 'admin' &&  <button onClick={makeadmin}  class="btn btn-primary">Make Admin</button>}</td>
       <label onClick={()=>setDoctorsmodal(doctor)}  for="DELETE_MODAL" class="btn  btn-secondary btn-xs">delete</label>
      <td>{ role !== 'admin '  && <button onClick={()=>setremoveuser(user)} class="btn btn-secondary">Remove User</button>}</td>
    </tr>
    );
};
