...
const Navber = () => {
  ...
  const SearchButton = () => {
    ...
    console.log(searchtext)
     ...
     const url  = `http://localhost:5000/bookproduct?s=${searchtext}`
    console.log(url)
     //  fetch()
    //  .then(res=>res.json())
    //  .then(data =>)
  ...
  const Menubar = <>
   <div class="flex-none gap-2  sm:w-10 sm:mr-24  lg:w-20  lg:mr-60  " >
    <div class="form-control flex-row">
      <input id='search-field'  type="text" placeholder="Search" class="input input-bordered" />
       <input onClick={()=>SearchButton()} type="submit" className=' btn btn-primary ' value={'search'} />
    </div>
    </div>
